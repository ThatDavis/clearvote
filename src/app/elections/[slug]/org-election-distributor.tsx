...
import ElectionVoterRollList from './election-voter-roll-list'
...
export default function OrgElectionDistributor({ slug, orgSlug, locked }: Props) {
  ...
  async function handleDistribute() {
    ...
    const res = await fetch(`/api/elections/${slug}/distribute`, {
      method: 'POST',
    ...
    if (!res.ok) {
      setError(data.error || 'Failed to distribute election')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5">
      ...
      <p className="mt-1 text-sm text-zinc-500">
        {locked
          ? 'The voter roll is locked while the election is open.'
          : 'Add organization members to the voter roll for this election.'}
      </p>
      ...
      <div className="mt-4">
        ...
        <ElectionVoterRollList voters={voters} onRemove={handleRemove} locked={locked} />
      </div>
    </div>
  )
}
